...
impl<C: CryptoProvider> QuinnClient<C> {
    ...
    /// Process a network output from the state machine
    #[allow(dead_code)] // Will be used for future non-authentication messages
    async fn process_output(&mut self, output: NetworkOutput<'_>) -> Result<(), QuinnError> {
        match output {
            NetworkOutput::OpenUniStream => {
                // Quinn automatically opens streams on demand
                // No action needed
            }

            NetworkOutput::SendMessage(msg) => {
                // Send a structured message (CBOR-encode at transport boundary)
                if let Some(conn) = &self.connection {
                    let conn_id = format!("{:?}", conn.stable_id());
                    // CBOR encode the structured message
                    let cbor_bytes = openscreen_network::messages::encode_network_message(&msg)
                        .map_err(|_| NetworkError::EncodeFailed)?;
            ...
            NetworkOutput::SendData { data } => {
                ...
                if let Some(conn) = &self.connection {
                    ...
                    send_stream.finish()?;
                    info!(
                        "[CONN:{}][STREAM:{}] 📤 TX {} bytes (raw app data)",
                        conn_id,
                        send_stream_id.index(),
                        data.len()
                    );
            ...
            NetworkOutput::CloseStream(_stream_id) => {
                // Streams are automatically closed when dropped
                // No action needed
            }
            ...
            NetworkOutput::RequestCrypto(_) => {
                // RequestCrypto is handled specially in process_event
                // to avoid borrow checker issues. This should never be reached.
                unreachable!("RequestCrypto should be handled in process_event")
            }

            NetworkOutput::NeedsPoll => {
                // State machine needs immediate polling
                // In event loop, this would trigger another iteration
            }

            NetworkOutput::Event(_event) => {
                // Handle network events (Authenticated, AuthenticationFailed, etc.)
