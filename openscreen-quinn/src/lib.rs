...
mod tests {
    ...
    async fn test_client_cert_has_subject_cn() {
        // Verify that certificates generated by QuinnClient have Subject CN set per W3C spec
        let crypto = MockCryptoProvider::new(b"test");
        let expected_fp = [0u8; 32];
        let hostname = "test-hostname.local";
        let client = QuinnClient::new(crypto, "0.0.0.0:0".parse().unwrap(), expected_fp, hostname)
